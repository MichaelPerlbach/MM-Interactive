	...
	public	function	showajaxreturnAction(){
		...
		if($args['do'] == 'setorder'){
			...
			foreach($data as $item){
				...
				foreach($remdata as $remove){ $this->orderpartdataRepository->remove($remove); }
				...
				if(isset($item['producttype'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.product', 'Mminteractive'),
						'val' => $name
					);
				}
				...
				if(isset($item['completion'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.completion', 'Mminteractive'),
						'val' => $name
					);
				}
				...
				if(isset($item['material'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.material', 'Mminteractive'),
						'val' => $name
					);
				}
				...
				if(isset($item['materialdesc'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.materialdesc', 'Mminteractive'),
						'val' => $name
					);
				}
				...
				if(isset($item['productiondepth'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.productiondepth', 'Mminteractive'),
						'val' => $name
					);
				}
				...
				if(isset($item['implantsupplier'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.supplier', 'Mminteractive'),
						'val' => $name
					);
				}
				...
				if(isset($item['implantsystem'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.system', 'Mminteractive'),
						'val' => $name
					);
				}
				...
				if(isset($item['implantsize'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.mm', 'Mminteractive'),
						'val' => $name
					);
				}
				...
				if(isset($item['implantgingiva'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.gingiva', 'Mminteractive'),
						'val' => $name
					);
				}
				...
				if(isset($item['teeth']) && $item['teeth'] != '0'){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.teeth', 'Mminteractive'),
						'val' => $item['teeth']
					);
				}
				...
				if(isset($item['adw1'])){
					...
					$thisreturn['items'][] = array(
						'item'=>	\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.addwork', 'Mminteractive'),
						'val' => $name.' | '.$alanr
					);
				}
				...
				if(isset($item['color'])){
					...
					$thisreturn['items'][] = array(
						'item'=>\TYPO3\CMS\Extbase\Utility\LocalizationUtility::translate('tx_Mminteractive.color', 'Mminteractive'),
						'val' => $name
					);
				}
				$otext = 'Datei: '.$item['file']."\n";
				foreach($thisreturn['items'] as $o_item){ $otext .= implode(': ',$o_item)."\n"; }
		...
		if($args['do'] == 'sendorder'){
			...
			if(!preg_match("/test/i",$feuser['company'])){ $mainorder->setStatus(1); }
			...
			$adminmail = str_replace('<!-- ###DL### -->','<tr><td>Download der Dateien: <a href="https://argen-digital.de/orderdata/perm/'.$uploaddir.'/download/'.$uploaddir.'.zip">https://argen-digital.de/orderdata/perm/'.$uploaddir.'/download/'.$uploaddir.'.zip</a></td></tr>',$mail_main);
		...
		if($args['do'] == 'updateprod'){
			...
			$prods = array();
			foreach($products as $product){ $prods[] = $product['productid']; }
		...
		if($args['do'] == 'updatedata'){
			...
			if(count($prods) > 0){ $return['prods'] = implode(',',$prods); }
		}
		if(count($prods) > 0){
	...
	public function getfiles($dir,$shapes){
		...
		foreach($objects as $name => $object){
			...
			if(in_array(strtolower($ext),$shapes)){
				if(!in_array(strtolower($ext),$realshapes)){ $realshapes[] = strtolower($ext); }
			}
		}
...
function fcopy($src,$dst) { 
	...
	while(false !== ( $file = readdir($dir)) ) { 
		if (( $file != '.' ) && ( $file != '..' )) {
			if(!is_dir($src . '/' . $file)) { rename($src . '/' . $file,$dst . '/' . $file); }
		}
	} 
	closedir($dir); 
} 

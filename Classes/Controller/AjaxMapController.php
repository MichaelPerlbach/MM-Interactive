...
class    Tx_mminteractive_Controller_AjaxMapController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
    ...
    public function loadlayoutAction()
    {
        $args = $this->request->getArguments();
        $id = $args['id'];
        $thislayout = $this->customerlayoutRepository->findLayout((int)$id);                /*		$abspath = \MikelMade\Mminteractive\Utility\Tx_Mminteractive_Utility_Div::absPath();		$fp = fopen($abspath.'webseiten/debug.txt','w+');		fputs($fp,json_encode($thislayout));		fclose($fp);		*/
        print json_encode($thislayout);
    }
    ...
    public function savelayoutAction()
    {
        $args = $this->request->getArguments();
        $id = $args['id'];
        $thislayout = $this->customerlayoutRepository->findByUid((int)$id);
        $thislayout->setTitle($args['title']);
        $thislayout->setFont($args['font']);
        $thislayout->setFontsize($args['fontsize']);
        $thislayout->setFontcolor($args['fontcolor']);
        $thislayout->setHeadlinecolor($args['headlinecolor']);
        $thislayout->setBackgroundcolor($args['backgroundcolor']);
        $thislayout->setLinkColorActive($args['linkcoloractive']);
        $thislayout->setLinkColorInactive($args['linkcolorinactive']);
        $thislayout->setLinkColorrollover($args['linkcolorrollover']);
        $thislayout->setPicframe($args['picframe']);
        $thislayout->setPicframecolor($args['picframecolor']);
        $thislayout->setArrowcolor($args['arrowcolor']);
        $thislayout->setHeadersize($args['headersize']);
        $thislayout->setLinecolor($args['linecolor']);
        $thislayout->setUpperedgeColor($args['upperedgecolor']);
        $thislayout->setIconsColorActive($args['iconscoloractive']);
        $thislayout->setIconsColorInactive($args['iconscolorinactive']);
        $thislayout->setIconsColorRollover($args['iconscolorrollover']);
        $this->customerlayoutRepository->update($thislayout);
        $persistenceManager = $this->objectManager->get('TYPO3\\CMS\\Extbase\\Persistence\\Generic\\PersistenceManager');
        $persistenceManager->persistAll();
    }
    ...
    public function deletelayoutAction()
    {
        ...
        $custid = $args['custid'];
        $thislayout = $this->customerlayoutRepository->findByUid((int)$id);
        $thislayout->setDeleted(1);
        $this->customerlayoutRepository->update($thislayout);
        $persistenceManager = $this->objectManager->get('TYPO3\\CMS\\Extbase\\Persistence\\Generic\\PersistenceManager');
        $persistenceManager->persistAll();
        $data = $this->customerlayoutRepository->findLayouts((int)$custid);
    ...
    public function addvorlageAction()
    {
        $args = $this->request->getArguments();
        $custid = $args['custid'];
        $thislayout = new MikelMade\Mminteractive\Domain\Model\Customerlayout();
        $thislayout->setCustomerid($custid);
        $thislayout->setTitle($args['title']);
        $thislayout->setFont($args['font']);
        $thislayout->setFontsize($args['fontsize']);
        $thislayout->setFontcolor($args['fontcolor']);
        ...
        $thislayout->setIconsColorInactive($args['iconscolorinactive']);
        $thislayout->setIconsColorRollover($args['iconscolorrollover']);
        $this->customerlayoutRepository->add($thislayout);
        $persistenceManager = $this->objectManager->get('TYPO3\\CMS\\Extbase\\Persistence\\Generic\\PersistenceManager');
        $persistenceManager->persistAll();
        $data = array(
            'layoutid' => $thislayout->getUid(),
            'layouts' => $this->customerlayoutRepository->findLayouts((int)$custid)
        );
        print json_encode($data);
    }
}
